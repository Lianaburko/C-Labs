...
namespace Lab7
{
    public class Fraction : IComparable<Fraction>, IFormattable, IConvertible, ICloneable
    {
        ...
        private static long GCD(long a, long b) // Greated Common Divisor 
        {
            a = Math.Abs(a);
            b = Math.Abs(b);
            while (a > 0 && b > 0)
        ...
        public static bool TryParse(String s, out Fraction fract)
        {
            fract = null;
            ...
            {s
                ...
                fract = new Fraction(sign * (Math.Abs(integer) * den + num), den);
                return true;
            }
            if (doublePattern.IsMatch(s))
            {
                ...
                fract = new Fraction((int)num, den);
                return true;
            }
